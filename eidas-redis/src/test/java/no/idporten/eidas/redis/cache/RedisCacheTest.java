...
public class RedisCacheTest {
    ...
    @Test
    public void testReplaceMatchingValue() {
        when(redisTemplate.hasKey("test:key1")).thenReturn(true);
        when(valueOperations.get("test:key1")).thenReturn("oldValue");
        assertTrue(cache.replace("key1", "oldValue", "newValue"));
        verify(valueOperations).set("test:key1", "newValue", 300, TimeUnit.SECONDS);
    }

    @Test
    public void testReplaceNonMatchingValue() {
        when(redisTemplate.hasKey("test:key1")).thenReturn(true);
        when(valueOperations.get("test:key1")).thenReturn("otherValue");
        assertFalse(cache.replace("key1", "oldValue", "newValue"));
    }
    ...
    public void testReplaceSimple() {
        when(valueOperations.get("test:key1")).thenReturn("oldValue");
        assertTrue(cache.replace("key1", "newValue"));
    }

    @Test
    public void testGetAndReplace() {
        when(valueOperations.get("test:key1")).thenReturn("oldValue");
        assertEquals("oldValue", cache.getAndReplace("key1", "newValue"));
    }
    ...
    public void testRemoveAll() {
        ...
        verify(redisTemplate).delete("test:k1");
        verify(redisTemplate).delete("test:k2");
    }

    @Test
    public void testRemoveAllNoArgs() {
        Set<String> keys = new HashSet<>(Arrays.asList("test:k1", "test:k2"));
    ...
    @Test
    public void testClearEmptyKeys() {
    ...
    @Test(expected = UnsupportedOperationException.class)
    public void testLoadAllUnsupported() {
    ...
    @Test(expected = UnsupportedOperationException.class)
    public void testGetConfigurationUnsupported() {
        cache.getConfiguration(null);
    }

    @Test(expected = UnsupportedOperationException.class)
    public void testInvokeUnsupported() {
        cache.invoke(null, null);
    }

    @Test(expected = UnsupportedOperationException.class)
    public void testInvokeAllUnsupported() {
        cache.invokeAll(null, null);
    }
    ...
    @Test(expected = UnsupportedOperationException.class)
    public void testRegisterCacheEntryListenerUnsupported() {
        cache.registerCacheEntryListener(null);
    }

    @Test(expected = UnsupportedOperationException.class)
    public void testDeregisterCacheEntryListenerUnsupported() {
